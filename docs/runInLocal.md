...
- [1. 任意系统，但已安装`.NET 10.0`](#1-任意系统但已安装net-100)
- [5. 配置](#5-配置)
...
## 1. 任意系统，但已安装`.NET 10.0`

任何操作系统，不管是Win还是Linux还是mac，只要已安装了`.NET 10.0` 环境，均可通过下载`net-dependent.zip`运行。
...
P.S.这里的运行环境指的是 `.NET Runtime 10.0.0` ，安装方法可详见 [常见问题](questions.md) 中的 **本地或服务器如何安装.net环境**
