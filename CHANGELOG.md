...
- Fix[#1147][#1149]: 补齐 #1138 目录迁移后仍遗漏的旧路径。白虎 `bili_task_base.sh` 按固定层级上跳定位仓库根，少一级后 `cd <repo>/src/...` 指到 `platforms/` 下，白虎全部任务失败；`tencentScf/publish.sh|.bat` 的 `cd ../src` 同样失效，`auto-deploy-tencent-scf` 定时部署中断；青龙 `bili_task_tryFix.sh` 找不到 `ray-dotnet-install.sh`，需要装 .NET 时修复任务必然失败；docker/podman 的 5 个 build 脚本构建上下文指向 `platforms/`；5 个平台 README 共 30 处 `../docs/...` 链接因多套一层目录而失效
## 4.0.6
